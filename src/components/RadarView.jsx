...
export default function RadarView({ title = 'Radar Sensor', streamUrl = null, onFeedStatus }) {
    ...
    <div className="panel radar-view">
      ...
      <div className="panel__body radar-view__body">
        {streamUrl && feedAlive ? (
          <img
            src={streamUrl}
            className="radar-view__feed"
            alt={title}
            onLoad={() => setAlive(true)}
            onError={() => setAlive(false)}
          />
        ) : (
          <div className="radar-view__overlay">{streamUrl ? 'Not Connected' : 'Not Installed'}</div>
        )}
      </div>
