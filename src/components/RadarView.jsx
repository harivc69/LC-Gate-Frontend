...
export default function RadarView({ title = 'Radar Sensor', streamUrl = null }) {
      ...
      <div className="panel__header">
        ...
        <div className="panel__meta">
          ...
          <span className={`panel__status ${streamUrl ? 'panel__status--ok' : 'panel__status--fault'}`}>
            {streamUrl ? 'ACTIVE' : 'OFFLINE'}
          </span>
        </div>
      </div>
      <div className="panel__body radar-view__body">
        {streamUrl ? (
          <img src={streamUrl} className="radar-view__feed" alt={title} />
        ) : (
          <div className="radar-view__overlay">Not Installed</div>
        )}
      </div>
